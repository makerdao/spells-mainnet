...
import {DssSpell, SpellAction} from "./PSMDssSpell.sol";
...
contract DssSpellTest is DSTest, DSMath {
    ...
    DaiAbstract     dai         = DaiAbstract(      0x6B175474E89094C44Da98b954EedeAC495271d0F);
    address         flipperMom  =                   0x9BdDB99625A711bf9bda237044924E34E8570f75;
    GemJoinAbstract pUAJoin     = GemJoinAbstract(  address(0));
    FlipAbstract    pUAFlip     = FlipAbstract(     address(0));
    PsmAbstract     pUA         = PsmAbstract(      address(0));
    DSTokenAbstract usdc        = DSTokenAbstract(  0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);
    ...
    function testSpellIsCast() public {
        ...
        checkCollateralValues("PSM-USDC-A", afterSpell);
    }

    function testNewCollateral_PSM_USDC_A() public {
        vote();
        scheduleWaitAndCast();
        assertTrue(spell.done());

        assertEq(pUA.feeIn(), 0.1 * 10 ** 27 / 100);
        assertEq(pUA.feeOut(), 0.1 * 10 ** 27 / 100);

        // Authorization
        assertEq(pUAJoin.wards(address(pUA)), 1);
        assertEq(vat.wards(address(pUAJoin)), 1);
        assertEq(pUAFlip.wards(address(cat)), 1);
        assertEq(pUAFlip.wards(address(end)), 1);
        assertEq(pUAFlip.wards(flipperMom), 1);
        //

        uint256 daiBalance = dai.balanceOf(address(this));
        uint256 usdcBalance = usdc.balanceOf(address(this));

        uint256 usdcToJoin = 40 * 10 ** 6;
        usdc.approve(address(pUA), usdcToJoin);
        pUA.join(usdcToJoin);

        daiBalance = daiBalance + (usdcToJoin * 10 ** 12 - usdcToJoin * 10 ** 12 / 1000);
        usdcBalance = usdcBalance - usdcToJoin;
        assertEq(dai.balanceOf(address(this)), daiBalance);
        assertEq(usdc.balanceOf(address(this)), usdcBalance);

        uint256 usdcToExit = 40 * 10 ** 6;
        dai.approve(address(pUA), usdcToExit * 10 ** 12 + usdcToExit * 10 ** 12 / 1000);
        pUA.exit(usdcToExit);

        daiBalance = daiBalance - (usdcToExit * 10 ** 12 + usdcToExit * 10 ** 12 / 1000);
        usdcBalance = usdcBalance + usdcToExit;
        assertEq(dai.balanceOf(address(this)), daiBalance);
        assertEq(usdc.balanceOf(address(this)), usdcBalance);
    }
}
