...
contract DssSpellTest is DSTest, DSMath {
    ...
    function testNewCollateral() public {
        ...
        // WBTC-A fee
        (uint256 dutyWBTC,) = jug.ilks("WBTC-A");
        assertEq(dutyWBTC, 1000000005781378656804591712);
        assertTrue(diffCalc(expectedRate(20 * 1000), yearlyYield(dutyWBTC)) <= TOLERANCE);

        // WBTC-A line and dust
        (,,, uint256 lineWBTC, uint256 dustWBTC) = vat.ilks("WBTC-A");
        assertEq(lineWBTC, 20 * MILLION * RAD);
        assertEq(dustWBTC, 20 * RAD);

        // WBTC-A liquidation penalty and lot size
        (address aux, uint256 chop, uint256 lump) = cat.ilks("WBTC-A");
        FlipAbstract uFlip = FlipAbstract(aux);
        assertEq(chop, 113 * RAY / 100);
        assertEq(lump, 50 * THOUSAND * WAD);

        // WBTC-A percentage between bids
        assertEq(uFlip.beg(), 103 * WAD / 100);
        // WBTC-A max time between bids
        assertEq(uint256(uFlip.ttl()), 6 hours);
        // WBTC-A max auction duration
        assertEq(uint256(uFlip.tau()), 3 days);

        // WBTC-A min collateralization ratio
        (, uint256 mat) = spot.ilks("WBTC-A");
        assertEq(mat, 125 * RAY / 100);

        // Line
        assertEq(vat.Line(), 133 * MILLION * RAD);

        // WBTC Pip => 1 WBTC == 1 DAI
        assertEq(uint256(wPip.read()), 1 * WAD);
        // WBTC Pip Owner
        assertEq(wPip.owner(), pauseProxy);
        // WBTC Pip Authority
        assertEq(wPip.authority(), address(0));

        // Authorization
        assertEq(wJoin.wards(pauseProxy), 1);
        assertEq(vat.wards(address(wJoin)), 1);
        assertEq(wFlip.wards(address(cat)), 0); // FlipperMom denied it at the end of the spell (no liquidations on first phase)
